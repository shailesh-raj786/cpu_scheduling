...
const string ALGORITHMS[5] = {"", "FCFS", "RR-", "SJF", "SRT"};
...
void firstComeFirstServe(){
    ...
    for (int i = 0; i < process_count; i++){
        ...
        for (int j = time; j < finishTime[processIndex]; j++){
            timeline[j][processIndex] = '*';   // execution
        }
        for (int j = arrivalTime; j < time; j++){
            timeline[j][processIndex] = '.'; // wait for execution
        }
        time += serviceTime;
    }
}
...
void roundRobin(int originalQuantum){
    ...
    for(int time =0;time<last_instant;time++){
        if(!q.empty()){
            ...
            q.front().second = q.front().second-1;  // ek baar chala chuke yaha tak
...
void printTimeline(){
    ...
    cout << "------------------------------------------------\n";
    for(int i = 0; i < process_count; i++){
        cout << getProcessName(processes[i]) << "     |"; // getProcessName will give us the id of the process like a,b,c,d,e etc
